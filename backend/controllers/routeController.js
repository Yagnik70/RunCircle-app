...
export const createRoute = (req, res) => {
  ...
  if (distance_km > MAX_DISTANCE_KM) {
    return res
      .status(400)
      .json({
        message: `Route distance cannot exceed ${MAX_DISTANCE_KM} km within Ahmedabad`,
      });
  }

  const sql =
    "INSERT INTO routes (user_id, from_location, to_location, distance_km) VALUES (?, ?, ?, ?)";
...
export const saveRoute = (req, res) => {
  ...
  const sql =
    "INSERT INTO saved_routes (user_id, route_id, sport_id) VALUES (?, ?, ?)";
